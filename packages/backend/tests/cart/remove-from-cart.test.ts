...
describe("Remove from cart", () => {
    ...
    it("Successfully removes a product from cart", async () => {
        ...
        const removeRes = await agent.delete(`/api/cart/remove?productId=${productId}`);
    ...
    it("Returns 404 when cart doesn't exist", async () => {
        ...
        // Signup a new user with no cart
        const signupRes = await request(app).post("/api/auth/signup").send({
            name: "No Cart User",
        ...
        await agent2.post("/api/auth/signin").send({
            email: "nocart@mail.com",
            password: "testpassword",
        });

        const removeRes = await agent2.delete("/api/cart/remove?productId=1");
    ...
    it("Returns 404 when product is not in cart", async () => {
        ...
        const removeRes = await agent.delete("/api/cart/remove?productId=9999");
        expect(removeRes.statusCode).toBe(404);
    });
});